...
`powered_rail`; at a line's end that relaunches an overshooting cart off the
edge (the original fall). The terminus is dead simple: the track **ends at a
solid 2-high wall** one block ahead. A cart bumps the wall and stops — verified
it halts cleanly even at an over-max Motion 0.8, no clip-through — and the wall
keeps you from walking off the edge. No forward rail, no lever.

(Lesson, the hard way: an over-built version used a multi-rail "brake zone" to
stop the cart gently, on a wrong theory that a wall impact throws the rider. It
doesn't — a plain block in front of the rail end just works, as the simplest
thing would suggest. Don't out-think the buffer.)

To leave, nudge the cart back the way it came; the line's boosters are
bidirectional and carry it.
